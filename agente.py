...
class Agente:
    ...
    def __init__(self, id, tam_agente, cor=None, cop=False):
        ...
        if cor:
            ...
            self._cor = choice(['gold', 'orange', 'red', 'maroon', 'violet', 'magenta', 'purple', 'navy', 'blue', 'skyblue', 'cyan', 'turquoise', 'lightgreen', 'green', 'darkgreen', 'chocolate', 'brown', 'black', 'gray'])
        # REQ OK
    ...
    def desenhar_se(self, posicao=None):
        ...
        self._turtle.down()
        if self._cop:
            self._turtle.dot(self._tam_agente, 'white')
            self._turtle.dot(self._tam_agente / 2, 'red')
        else:
    ...
    def vaguear(self):
        ...
        prox_pos_agente = (
            self._posicao.coord_turtle()[0] + self.prox_passo()[0],
            self._posicao.coord_turtle()[1] + self.prox_passo()[1]
        )

        # print('Pos', self._posicao.coord_turtle()[0], self._posicao.coord_turtle()[1])
        # print('Prox', self._posicao.coord_turtle()[0] + self.prox_passo()[0], self._posicao.coord_turtle()[1] + self.prox_passo()[1])
        # print('caminho', lab.eh_caminho(prox_pos_agente[0], prox_pos_agente[1]))
        # print('ocupada', lab.eh_celula_ocupada(prox_pos_agente[0], prox_pos_agente[1]))

        while not lab.eh_caminho(prox_pos_agente[0], prox_pos_agente[1]) or lab.eh_celula_ocupada(prox_pos_agente[0], prox_pos_agente[1]):
        ...
        self._posicao = lab.criar_celula(coord_turt=prox_pos_agente)
